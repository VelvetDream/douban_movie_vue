<template>
    <el-dialog id="register-component" title="注册" :visible="isRegistering">
      <el-form :model="registerForm" ref="registerForm" :rules="registerFormRules">
        ...
        <el-input type="password" v-model="registerForm.passwordRepeat"  placeholder="请再次输入密码"  show-password/>
        </el-form-item>
                <el-form-item>
                  <span class="serverError">{{serverError}}</span>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="isLoading" @click="submitRegisterForm">注册</el-button>
...
export default {
  ...
  data () {
    ...
    var validatorPassword = (rule, value, callback) => {
      ...
      } else if (value !== this.registerForm.passwordRepeat && this.registerForm.passwordRepeat !== '') {
